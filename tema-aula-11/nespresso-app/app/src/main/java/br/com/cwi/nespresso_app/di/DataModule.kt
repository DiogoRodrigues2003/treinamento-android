...
import br.com.cwi.nespresso_app.data.network.mapper.AccessoryMapper
...
import br.com.cwi.nespresso_app.data.repository.AccessoryLocalRepositoryImpl
...
val dataModule = module {
    ...
    single { AccessoryMapper() }
    single { AppDatabase.create(androidApplication()) }

    factory<CoffeeRepository> { CoffeeRepositoryImpl(get(), get(), get(), get(), get()) }
    factory<CoffeeLocalRepository> { CoffeeLocalRepositoryImpl(get()) }
    factory<AccessoryLocalRepository> { AccessoryLocalRepositoryImpl(get()) }
}