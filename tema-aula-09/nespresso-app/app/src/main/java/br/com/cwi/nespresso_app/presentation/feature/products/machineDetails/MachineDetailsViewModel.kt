...
class MachineDetailsViewModel(private val repository: CoffeeRepository): BaseViewModel() {

    private val _machine = MutableLiveData<Machine>()
    val machine: LiveData<Machine> = _machine

    fun fetchMachineDetails(itemId: Int) {
        launch {
            val selectedMachine = repository.getMachineById(itemId)
